...
const Page = () => {
    const { idioma, idiomas, diferencial } = usePage().props;

    const { data, setData, post, processing, errors } = useForm(diferencial);

    const breadcrumbItems = [
        { label: 'Institucional', link: 'Manager.Institucional.index' },
        { label: 'Diferenciais', link: 'Manager.Institucional.index' },
    ];
    ...
    const handleSubmit = (e) => {
        ...
        post(route('Manager.Diferenciais.atualizar', {id: diferencial.id, lang: idioma_url}), {
            preserveScroll: true,
        });
        console.log(data);
    ...
    return (
        <AdminLayout>
            <Breadcrumb icon={faList} items={breadcrumbItems} current="Editar" idioma={idioma.codigo} idiomas={idiomas} id={diferencial.id} />
