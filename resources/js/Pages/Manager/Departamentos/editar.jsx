...
const Page = () => {
    const { idioma, idiomas, departamento } = usePage().props;

    const { data, setData, post, processing, errors } = useForm(departamento);

    const breadcrumbItems = [
        { label: 'Contato', link: 'Manager.Contato.index' },
        { label: 'Departamentos', link: 'Manager.Contato.index' },
    ];

    const inputItems = [
        [{ titulo: 'Nome', name: 'nome', tamanho: 'col-span-12 lg:col-span-8', tipo: 'texto', max: 120 }],
        [{ titulo: 'E-mails', name: 'emails', tamanho: 'col-span-12 lg:col-span-8', tipo: 'tag' }]
    ];
    ...
    const handleSubmit = (e) => {
        ...
        post(route('Manager.Departamentos.atualizar', {id: departamento.id, lang: idioma_url}), {
            preserveScroll: true,
        });
        console.log(data);
    ...
    return (
        <AdminLayout>
            <Breadcrumb icon={faList} items={breadcrumbItems} current="Editar" idioma={idioma.codigo} idiomas={idiomas} id={departamento.id} />
