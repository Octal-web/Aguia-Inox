...
const Page = () => {
    const { idioma, idiomas, imagem, produtos } = usePage().props;

    const { data, setData, post, processing, errors } = useForm(imagem);

    const breadcrumbItems = [
        { label: 'Segmentos', link: 'Manager.Segmentos.index' },
        { label: 'Produtos', link: 'Manager.Produtos.index' },
        { label: 'Editar Produto', link: 'Manager.Produtos.editar', params: { id: imagem.produto_id }},
    ];

    const inputItems = [
        [{ titulo: 'Título', name: 'titulo', tamanho: 'col-span-12 lg:col-span-8', tipo: 'texto', max: 80 }],
        [{ titulo: 'Texto', name: 'texto', tamanho: 'col-span-12 lg:col-span-8', tipo: 'texto_longo', editor: false, max: 1080 }],
        [{ titulo: 'Imagem', name: 'img', tamanho: 'col-span-12 lg:col-span-8', tipo: 'imagem', crop: true, largura: 900, altura: 1200, imagem: imagem.imagem }]
    ];
    ...
    const handleSubmit = (e) => {
        ...
        post(route('Manager.Produtos.Imagens.atualizar', {id: imagem.id, lang: idioma_url}), {
            preserveScroll: true,
        });
        console.log(data);
    ...
    return (
        <AdminLayout>
            <Breadcrumb icon={faImage} items={breadcrumbItems} current="Editar" idioma={idioma.codigo} idiomas={idiomas} id={imagem.id} />
