...
const Page = () => {
    const { idioma, idiomas, categoria, segmentos } = usePage().props;

    const { data, setData, post, processing, errors } = useForm(categoria);

    const breadcrumbItems = [
        { label: 'Segmentos', link: 'Manager.Segmentos.index' },
        { label: 'Categorias', link: 'Manager.Segmentos.index' },
    ];

    const inputItems = [
        [{ titulo: 'Nome', name: 'nome', tamanho: 'col-span-12 md:col-span-6 lg:col-span-4', tipo: 'texto', max: 120 }, { titulo: 'Segmento', name: 'segmento_id', tamanho: 'col-span-12 md:col-span-6 lg:col-span-4', tipo: 'select', options: segmentos }],
    ];
    const handleSubmit = (e) => {
        e.preventDefault();
        const idioma_url = new URLSearchParams(window.location.search).get('lang');

        post(route('Manager.Produtos.Categorias.atualizar', {id: categoria.id, lang: idioma_url}), {
            preserveScroll: true,
        });
        console.log(data);
